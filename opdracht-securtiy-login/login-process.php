<?php
	...
	if(isset($_POST["submit"])) 
	{
		...
		$queryuser = 'SELECT email,salt,hased_password FROM users WHERE email=:currentemail';
		...
		if($passwordhas == $result["hased_password"]) 
		{
			setcookie("login", $email.",".$result["hased_password"].$result["salt"], time() + 2592000);
			header("Location: dashboard.php");
			exit();
		}
		else
